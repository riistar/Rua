...
uses
  ...
  uProfiles, Nexon3PAPIImport, Vcl.Imaging.pngimage,
...
type
  ...
  private
    ...
    procedure GameExited(Sender: TObject);
...
uses
  ...
  frmLogin, frmLoginWebView, frmProfile, frmProfileEdit, frmSettings, frmFolderSelect, frmAbout,
  uNxlPatcher, uCredStore;
...
begin
  ...
  FGameExitHandler := GameExited;

  NP_Init(PAnsiChar(AnsiString(TPath.Combine(GetEnvironmentVariable('APPDATA'), 'Rua'))), nil);
  ...
  // About menu item (between Settings and Exit)
  var MenuAbout := TMenuItem.Create(MainMenu);
  MenuAbout.Caption := 'About';
  MenuAbout.OnClick := MenuAboutClick;
  MainMenu.Items.Insert(MainMenu.Items.IndexOf(MenuExit), MenuAbout);
...
begin
  ...
  P.Name     := Name;
  P.UserNo   := NP_ExtractCookie(Cookies, 'NexonUserID');
  P.DeviceId := NP_GetDeviceId(Name);
  ...
  begin
    ...
    for var K in ['NxLSession','AToken','g_AToken','NxGUN','NexonUserID','id_token'] do
      if NP_ExtractCookie(Cookies, K) <> '' then
        Keys := Keys + K + ' ';
...
begin
  ...
  for var K in ['NxLSession','AToken','NxGUN','g_AToken','NexonUserID'] do
    if NP_ExtractCookie(Cookies, K) <> '' then Keys := Keys + K + ' ';
...
var
  F: frmAbout.TFormAbout;
begin
  F := frmAbout.TFormAbout.Create(nil);
  try
    F.ShowModal;
  finally
    F.Free;
...
begin
  ...
  Merged := Fresh;
  if NP_ExtractCookie(Merged, 'NxLSession') = '' then
  begin
    var Old := LoadCookies(Profile);
    var OldNxL := NP_ExtractCookie(Old, 'NxLSession');
...
begin
  ...
  if NP_CheckSession(Cookies, Status, NexonCode) then
  begin
  ...
  begin
    ...
    Refreshed := NP_AutoLoginRefresh(Cookies, DevId);
    if Refreshed <> '' then
    begin
...
begin
  ...
  begin
    ...
    try
      try
        RemoteHash := NP_FetchManifestHash(Cookies, ProductId);
      except
        on E: ENP_UpdateCheckError do
        begin
...
procedure TFormMain.GameExited(Sender: TObject);
begin
  Log('Game exited.');
...
var
  ...
  ProductId, LaunchCode: Integer;
  function DoLaunch: Integer;
  begin
    Result := NP_LaunchGame(Cookies, ProductId, GamePath, '', nil,
      procedure begin
        TThread.Queue(nil, procedure begin
          if Assigned(FGameExitHandler) then FGameExitHandler(Self);
        end);
      end);
  end;
begin
  ...
  try
    LaunchCode := DoLaunch;
    if LaunchCode = NP_ErrSession then
    begin
      ...
      begin
        ...
        if Cookies <> '' then
          ...
          raise Exception.Create('No credentials available');
      end;
    end;
    if LaunchCode <> NP_OK then
      raise Exception.CreateFmt('Launch failed (code %d)', [LaunchCode]);
    UpdateLastUsed(Name);
...
begin
  ...
  var
    ...
    Reason:  string;
  begin
    Valid             := NP_CheckSession(Cookies, Status, NexonCode);
    NewInfo.Valid     := Valid;
...
begin
  ...
  if NP_CheckSession(NewCookies, Status) then
  begin
