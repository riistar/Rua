...
begin
  ...
  TpaSession := NP_ExtractCookie(RawCookies, 'TpaSession');
  if TpaSession <> '' then
  begin
    LblStatus.Caption := 'Exchanging TpaSession → NxLSession...';
    Application.ProcessMessages;

    FCookies := NP_ExchangeTpa(TpaSession, NP_GetDeviceId(FProfileName));
    if FCookies = '' then
    begin
      LblStatus.Caption := 'Exchange failed. ' +
    ...
    var IdToken := NP_ExtractCookie(RawCookies, 'id_token');
    if (IdToken <> '') and (Pos('id_token', FCookies) = 0) then
      FCookies := FCookies + '; id_token=' + IdToken;

    LblStatus.Caption := 'Logged in via ' + NP_BrowserName(Browser) + '. Closing...';
    FLoginDone    := True;
    ModalResult   := mrOK;
    Exit;
  end;
  ...
  begin
    FCookies          := RawCookies;
    LblStatus.Caption := 'Session found via ' + NP_BrowserName(Browser) + '. Closing...';
    FLoginDone        := True;
    ModalResult       := mrOK;
    Exit;
  end;
...
begin
  ...
  begin
    try
      NP_GetFirefoxCookies(FFCookies);
    except
      FFCookies := '';
    end;
    ...
    begin
      ...
      NewTpa := NP_ExtractCookie(FFCookies, 'TpaSession');
...
begin
  ...
  begin
    RawCookies := '';
    if not NP_GetNexonCookies(RawCookies, Browser) then
    begin
      TThread.Queue(nil, procedure
      begin
        if FDestroying then Exit;
        LblStatus.Caption := 'No nexon.com session cookies found. Log in to nexon.com in Firefox, then click Import again, or use email/password above.';
        BtnImport.Enabled := True;
        TimerPoll.Enabled := True;
      end);
      Exit;
...
begin
  ...
  if FCookies = '' then
  begin
    LblStatus.Caption := 'Exchange failed. TpaSession may be expired — re-login and retry.';
...
begin
  ...
  begin
    ...
    Application.ProcessMessages;
    try
      Cookies  := NP_LoginOTP(FMfaKey, Otp, NP_GetDeviceId(FProfileName));
      FCookies := Cookies;
      LblStatus.Caption := 'Logged in. Closing...';
      ModalResult := mrOK;
    except
      on E: ENP_LoginFailed do
      begin
  ...
  Application.ProcessMessages;
  try
    Cookies  := NP_LoginEmailPassword(Email, Pwd, NP_GetDeviceId(FProfileName));
    FCookies := Cookies;
    LblStatus.Caption := 'Logged in. Closing...';
    ModalResult := mrOK;
  except
    on E: ENP_MfaRequired do
    begin
      FMfaKey := E.MfaKey;
