...
begin
  ...
  if IsPostLogin then
  begin
    if (Pos('NxLSession', Raw) = 0) and (NP_ExtractCookie(Raw, 'TpaSession') = '') then
    begin
      ...
      begin
        ...
        var Merged := NP_FetchAccess(Raw, 10200);
        Merged := NP_FetchAccount(Merged);
        if (NP_ExtractCookie(Merged, 'AToken') <> '')
           and (NP_ExtractCookie(Raw, 'AToken') = '') then
        begin
          FCookies := Merged;
  ...
  begin
    ...
    FOldTpaSession := NP_ExtractCookie(Raw, 'TpaSession');
  ...
  TpaSession := NP_ExtractCookie(Raw, 'TpaSession');
  if TpaSession <> '' then
  begin
    ...
    begin
      ...
      TimerCookies.Enabled := False;
      FCookies              := Raw;
      if (NP_ExtractCookie(FCookies, 'AToken') = '')
         and (NP_ExtractCookie(FCookies, 'g_AToken') <> '') then
        FCookies := FCookies + '; AToken=' + NP_ExtractCookie(FCookies, 'g_AToken');
    ...
    begin
      var CVal := NP_ExtractCookie(Raw, CName);
  ...
  begin
    ...
    begin
      ...
      for var K in ['NxLSession', 'AToken', 'g_AToken', 'NxGUN', 'NexonUserID'] do
        if NP_ExtractCookie(Raw, K) <> '' then Keys := Keys + K + ' ';
    ...
    var Keys := '';
    for var K in ['NxLSession', 'AToken', 'g_AToken', 'NxGUN', 'NexonUserID'] do
      if NP_ExtractCookie(FCookies, K) <> '' then Keys := Keys + K + ' ';
